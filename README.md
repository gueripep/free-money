# 10-Bagger Stock Finder (PEA-PME)

A deep document analysis pipeline using **Gemini 3 Flash Preview** to identify high-growth stocks in the European PEA-PME market.

This project automates the process of finding "10-Bagger" candidates by filtering the entire Euronext PEA-PME index based on strict quantitative metrics (Market Cap, Float, Margins, Cash Flow, ROE, etc.), and then using a Streamlit "Cockpit" to manually grade them and run deep-dive forensic analysis on their Annual and Interim Reports using AI.

## Getting Started

### 1. Prerequisites
- Python 3.9+
- A Google API Key for Gemini.
- Install dependencies:
  ```powershell
  pip install -r requirements.txt
  ```
- Copy `.env.example` to `.env` and add your Gemini API key:
  ```
  GEMINI_API_KEY="your_api_key_here"
  ```

### 2. Step 1: Data Ingestion
1. **Manually download** the official Euronext PEA-PME Excel list from [Euronext's documentation](https://connect2.euronext.com/en/media/169).
2. Save the `.xlsx` file into the `data/` directory.
3. Run the ingest script to parse the Excel file and resolve ISINs into Yahoo Finance tickers:
  ```powershell
  python pipeline/01_ingest_pea_pme.py
  ```
*(This creates `data/tickers.json`)*

### 3. Step 2: Fetch Financials (Populate Database)
Run the fetching script. This will query Yahoo Finance for every ticker and populate the local SQLite database (`data/stocks.db`) with crucial metrics like Market Cap, Margins (Gross, Operating, Profit), Debt, Cash flow, and Valuation multiples. 
*(Note: It throttles requests to 1 req/sec to prevent rate limiting).*
  ```powershell
  python pipeline/02_fetch_financials.py
  ```

### 4. Step 3: Run the Dashboard
Fire up the Streamlit interface to start filtering and analyzing stocks.
  ```powershell
  python -m streamlit run ui/app.py
  ```

### 5. Step 4: Rank the Winners (Tier List)
Once you have used The Cockpit to assign a "🟢 Good" verdict to multiple candidates, you can generate a final, definitive hierarchical ranking using the 5-Dimension Methodology.
  ```powershell
  python pipeline/04_rank_good_companies.py
  ```
The output will be perfectly visible inside the app's "War Room (Tier List)" tab.

## Using The App 🛩️

The Streamlit app is divided into two sections via the sidebar:

### The Launchpad
A massive table view showing the top quantitative candidates across the PEA-PME. It strictly filters for companies with:
- Market Cap between $50M - $1B
- Float under 25M shares
- Gross Margins > 60%
- Positive Operating Cash Flow

### The Cockpit (Deep Dive)
This is your main workflow hub. 
1. **Select a Ticker**: Use the dropdown or Next/Previous buttons to cycle through candidates.
2. **Grade It**: Assign a manual verdict (🔴 Bad, 🟡 Maybe, 🟢 Good) based on your gut check.
3. **Lite Search**: Run a fast, PDF-less AI synthesis based purely on the Live Yahoo Finance ratios.
4. **Rocket Fuel Deep Dive**: 
    - Upload an Annual Report PDF.
    - Ask the AI a specific custom question if you have one.
    - Click "🚀 Run 'Rocket Fuel' Deep Dive" to have Gemini 3 extract the complete narrative, moat, red flags, and catalyst from the document.
5. **Quarterly Sell Signal Update**: Upload interim/half-year reports to ask the AI if the original thesis is deteriorating.

### The War Room (Tier List)
The final stage of the pipeline. Here you will see the full, competitive Multi-Factor Tier List scoring every "Good" candidate against each other to help finalize your portfolio allocation.

All markdown analyses generated by the AI are permanently saved locally in `data/companies/<TICKER>/` (and `data/findings/` for the tier list) and are instantly readable in the tabs.
